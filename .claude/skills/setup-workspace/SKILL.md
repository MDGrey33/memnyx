...
- **Fork overlay source of truth.** A fork's `CLAUDE.fork.md` content is authored at the **fork's repo root** (`<source>/CLAUDE.fork.md`) — deliberately outside `.claude/skills/`, which sync overwrites wholesale, so fork content is never clobbered by a skill sync. The bundled `templates/CLAUDE.fork.md.tmpl` is only a **bootstrap stub**: init/sync render the repo-root file when it exists and fall back to the stub only for a fork that has not authored one yet (`fork_overlay_source` resolves this precedence, shared by init, sync, and the base `@`-include so they never disagree).
...
3. **CLAUDE.md layers** (reconciled only on `--apply-all`) — the base `CLAUDE.md` is regenerated from its template and the `CLAUDE.fork.md` overlay from the fork's overlay source (repo-root `<source>/CLAUDE.fork.md` preferred, bootstrap stub as fallback); both are upstream-/fork-owned. `CLAUDE.local.md` is seeded only if missing and never overwritten (user-owned). Two guards hold all three layers: if the source's git origin can't be classified (canonical vs fork), or the base is a legacy monolith lacking the `## Layered overlays` marker, the layers are left untouched.
...
#### Migrating a pre-layered workspace (the one-time split)

A workspace created before the layered-`CLAUDE.md` feature has a monolithic base — no `## Layered overlays` marker — so the guard holds all three layers and sync reports `CLAUDE.md — SKIPPED: legacy monolithic ... run the one-time split first`. The guard only refuses; performing the split is the agent's job, because judging which content is the engineer's own (vs template boilerplate) is judgment, not a script. When you hit that skip message, walk the engineer through it:

1. **Back up** the current `CLAUDE.md` first — the base is about to be overwritten from the template, so an un-migrated edit would be lost.
2. **Move genuinely-custom content into `CLAUDE.local.md`** — anything the engineer added beyond the standard template (the filled `## Conventions` block, personal notes). Stub/empty template sections carry over nothing — fast-path them. `CLAUDE.local.md` is user-owned and sync never overwrites it, so this is the durable home.
3. **Confirm the plan with the engineer**, then run `sync --apply-all`: with custom content parked in local, the base regenerates cleanly (marker now present), `CLAUDE.fork.md` deploys from the fork's overlay source, and `CLAUDE.local.md` is left untouched.

Note the ordering for a workspace whose deployed `setup-workspace` scripts predate this feature: the first `sync` updates the scripts themselves (the running old process can't use the new layer logic mid-run), so the split + layer reconciliation take effect on the **next** `sync --apply-all`.
...
- **Never overwrite user content.** MEMORY.md, lessons-learned.md, identity, brag log, growth, gitignore, settings.json, and `CLAUDE.local.md` — once present, stay as the user has them. The base `CLAUDE.md` and the `CLAUDE.fork.md` overlay are upstream-/fork-owned (regenerated by sync from the base template and the fork's repo-root `CLAUDE.fork.md` respectively); workspace-specific instructions belong in `CLAUDE.local.md`, which sync never overwrites. Never hand-edit the workspace's `CLAUDE.fork.md` — sync overwrites it from the fork's source; edit the fork's repo-root `CLAUDE.fork.md` instead.
